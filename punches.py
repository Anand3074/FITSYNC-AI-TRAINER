...
def angles(lmlist,p1,p2,p3,p4,p5,p6,drawpoints):
    ...
    if len(lmlist)!= 0:
        ...
        if drawpoints == True:
            ...
            lefthandangle = math.degrees(math.atan2(y2-y3,x2-x3)-
                                        math.atan2(y2-y1, x2-x1))
            righthandangle = math.degrees(math.atan2(y6 - y5, x6 - x5) -
                                         math.atan2(y4 - y5, x4 - x5))
            lefthandangle = (lefthandangle * -1)
            # print(lefthandangle)
            print(righthandangle)
            #print(lefthandangle,righthandangle)
            left = np.interp(lefthandangle, [28, 183], [0, 100])
            righthandangle = np.interp(righthandangle, [20, 177], [0, 100])
            # print(lefthandangle, righthandangle)
            # print(lefthandangle)


            left, right = lefthandangle, righthandangle
            # print(lefthandangle, left)
            # print(left)
            ...
            leftval = np.interp(left,[0,100],[400,200])
            # rightval = np.interp(right, [0, 100], [400, 200])
            reference_range = [400, 200]
            percentage = ((leftval - reference_range[1]) / (reference_range[0] - reference_range[1])) * 100
            ...
            cv2.rectangle(img, (28, int(leftval)), (70, 400), (255,0, 0), -1)
            ...
            if right > 70:
                cv2.rectangle(img, (28, int(leftval)), (70, 400), (0, 0, 255), -1)
